...
## See also
The model generated by either the Z Score or Min-Max function can be applied to any input data through the [`Existing Model Utilization`](https://www.docs.isalos.novamechanics.com/docs/analytics/existing-model-utilization.html) (e.g., a scaling function tuned from the training set data of a machine learning model can be applied to the test/external set data).
